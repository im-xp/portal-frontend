...
const Modal = ({open, onClose, title, description, children, className}: {open: boolean, onClose: () => void, title: string, description?: React.ReactNode, children: React.ReactNode, className?: string}) => {
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className={`bg-card text-card-foreground border-border rounded-lg p-6 max-w-md ${className}`}>
        <DialogHeader className="text-left pb-0">
          <DialogTitle className="text-lg font-semibold text-foreground leading-none tracking-tight">{title}</DialogTitle>
          {description && (
