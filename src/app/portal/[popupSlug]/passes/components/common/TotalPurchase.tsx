...
const TotalPurchase = ({ attendees, isModal, isOpen, setIsOpen }: {attendees: AttendeeProps[], isModal?: boolean, isOpen: boolean, setIsOpen: (prev: boolean) => void}) => {
  ...
  const productsCart = attendees.flatMap(attendee => {
    ...
    // Si hay un month seleccionado, mostrar los productos month, lodging, y donation
    if (hasMonthSelected) {
      return selectedProducts.filter(p => 
        p.category === 'month' || p.category === 'local month' || p.category === 'lodging' || p.category === 'donation'
      )
    }
    ...
    if (a.category === 'donation') return 1
    if (b.category === 'donation') return -1
  ...
  return (
    <Collapsible
      ...
      data-cart
    >
      <CollapsibleTrigger className={cn("w-full bg-secondary text-secondary-foreground rounded-md", isModal && "bg-transparent text-foreground")}>
