...
const success = ({arrivalDate, departureDate}: {arrivalDate: Date, departureDate: Date}) => {
  return (
    <motion.div
        ...
        >
          ...
          <div className="bg-gray-50 p-4 rounded-lg mb-6">
            <div className="grid grid-cols-2 gap-2 text-sm">
              ...
              <div className="font-medium text-right">{arrivalDate ? format(arrivalDate, "PPP") : ""}</div>
              <div className="text-gray-500 text-left">Departure Date:</div>
              <div className="font-medium text-right">{departureDate ? format(departureDate, "PPP") : ""}</div>
            </div>
          </div>

          <p className="text-sm text-gray-500">We look forward to seeing you soon in Healdsburg!</p>
        </motion.div>
  )
}
